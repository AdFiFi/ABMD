...
names = ["YAC", "YAG", "YAK", "YDG", "YDR", "YFR", "YFS", "YHS", "YIS",
...
class ZuCo2Dataset(BaseDataset):
    def __init__(self, args, data_config: DataConfig, k=0, train=True, subject_id=0):
        ...
        super(ZuCo2Dataset, self).__init__(args, data_config, k, train, subject_id=subject_id)
    ...
    def load_data(self):
        ...
        if "TSR" in self.data_config.data_dir:
            ...
            self.data_config.num_classes = 7
...
def zuco_preprocess_TSR(path=r"D:\data\ZuCo"):
    ...
    sentences = []
    sentence_ids = []
    label_path = os.path.join(path, "task_materials", f"tsr.csv")
    label_data = pd.read_csv(label_path, sep=";")
    sentence_list = label_data.sentence.tolist()
    ...
    for subject_id, name in enumerate(names):
        file_path = os.path.join(path, "task2 - TSR/Matlab files", f"results{name}1_TSR.mat")
        data = io.loadmat(file_path, squeeze_me=True, struct_as_record=False)['all_data']

        for i in range(len(data)):
            flag = True
            if isinstance(data[i].time_series, float) or data[i].time_series.shape[-1] < 500:
                continue
            ts = data[i].time_series[:104]
            ts[np.isnan(ts)] = 0
