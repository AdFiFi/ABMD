...
class MV2DTrainer(BMDTrainer):
    ...
    def init_model_config(self):
        if self.args.modality == 'vision-brain':
            ...
            mv2d_config = MV2DConfig(brain_config=brain_config,
            ...
            model = MV2D(config=mv2d_config,
                         brain_encoder=brain_encoder,
                         vision_encoder=vision_encoder)
            return model, mv2d_config
    ...
    def train_epoch(self, step_one=False):
        ...
        for step, inputs in enumerate(train_dataloader):
            ...
            loss_vcd += outputs.loss_vcd.item() if outputs.loss_vcd is not None else 0
            loss_vmd += outputs.loss_vmd.item() if outputs.loss_vmd is not None else 0
            wandb.log({'Loss Main/Training loss': loss.item(),
                       'Loss/Training VCD Loss': outputs.loss_vcd.item() if outputs.loss_vcd is not None else 0,
                       'Loss/Training VMD Loss': outputs.loss_vmd.item() if outputs.loss_vmd is not None else 0,
                       'Loss Main/Learning rate': self.optimizer.param_groups[0]['lr']})

        return losses / len(loss_list), (loss_vcd / len(loss_list), loss_vmd / len(loss_list))
    ...
    def train(self):
        ...
        for epoch in tqdm(range(1, self.args.num_epochs + 1), desc="epoch", ncols=0):
            start_time = timer()
            train_loss, (loss_vcd, loss_vmd) = self.train_epoch()
            end_time = timer()
            ...
            msg = f" Train loss: {train_loss:.5f}, " \
                  f"VCD Loss: {loss_vcd:.5f}, " \
                  f"VMD Loss: {loss_vmd:.5f}, " \
                  f"Test loss: {self.tva_result['Loss Main/Testing Loss']:.5f}, " \
                  f"VCD: {self.tva_result['Loss/Testing VCD Loss']:.5f}, " \
                  f"VMD: {self.tva_result['Loss/Testing VMD Loss']:.5f}, " \
    ...
    def two_step_train(self):
        ...
        for epoch in tqdm(range(1, self.args.num_first_step_epochs + 1), desc="first step epoch", ncols=0):
            start_time = timer()
            train_loss, (loss_vcd, loss_vmd) = self.train_epoch(step_one=True)
            end_time = timer()
    ...
    def binary_evaluate(self):
        ...
        with torch.no_grad():
            for inputs in evaluate_dataloader:
                ...
                loss = outputs.loss
                losses += loss.item()
                loss_vcd += outputs.loss_vcd.item()
                loss_vmd += outputs.loss_vcd.item()
                loss_list.append(loss.item())
            ...
            tva_result['Loss/Testing CVD Loss'] = loss_vcd / len(loss_list)
            tva_result['Loss/Testing VMD Loss'] = loss_vmd / len(loss_list)
    ...
    def multiple_evaluate(self, do_print=False):
        ...
        with torch.no_grad():
            for inputs in evaluate_dataloader:
                ...
                losses += loss.item()
                loss_vcd += outputs.loss_vcd.item()
                loss_vmd += outputs.loss_vmd.item()
                loss_list.append(loss.item())
            ...
            tva_result['Loss Main/Testing Loss'] = losses / len(loss_list)
            tva_result['Loss/Testing VCD Loss'] = loss_vcd / len(loss_list)
            tva_result['Loss/Testing VMD Loss'] = loss_vmd / len(loss_list)
