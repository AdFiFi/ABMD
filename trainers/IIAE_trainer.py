...
class IIAETrainer(Trainer):
    ...
    def init_model_config(self):
        if self.args.modality == 'vision-brain':
            ...
            IIAE_config = IIAEConfig(brain_config=brain_config,
            ...
            model = ModifiedIIAE(config=IIAE_config,
                                 brain_encoder=brain_encoder,
                                 vision_encoder=vision_encoder)
            return model, IIAE_config
    ...
    def train_epoch(self, step_one=False, pretrain=False):
        ...
        for step, inputs in enumerate(train_dataloader):
            ...
            outputs = self.model(**input_kwargs, step_one=step_one, pretrain=pretrain)
            ...
            reconstruction_loss += outputs.reconstruction_loss.item() if outputs.reconstruction_loss is not None else 0
            kl_loss += outputs.kl_loss.item() if outputs.kl_loss is not None else 0
            wandb.log({'Loss Main/Training loss': loss.item(),
                       'Loss/Training Reconstruction Loss': outputs.reconstruction_loss.item() if outputs.reconstruction_loss is not None else 0,
                       'Loss/Training KL Loss': outputs.kl_loss.item() if outputs.kl_loss is not None else 0,
                       'Loss Main/Learning rate': self.optimizer.param_groups[0]['lr']})

        return losses / len(loss_list), (reconstruction_loss / len(loss_list), kl_loss / len(loss_list))
    ...
    def train(self):
        ...
        for epoch in tqdm(range(1, self.args.num_epochs + 1), desc="epoch", ncols=0):
            ...
            msg = f" Train loss: {train_loss:.5f}, " \
                  f"Reconstruction: {train_loss2[0]:.5f}, " \
                  f"KL: {train_loss2[1]:.5f}, " \
                  f"Test loss: {self.tva_result['Loss Main/Testing Loss']:.5f}, " \
                  f"Reconstruction: {self.tva_result['Loss/Testing Reconstruction Loss']:.5f}, " \
                  f"KL: {self.tva_result['Loss/Testing KL Loss']:.5f}," \
    ...
    def two_step_train(self):
        ...
        for epoch in tqdm(range(1, self.args.num_first_step_epochs + 1), desc="first step epoch", ncols=0):
            start_time = timer()
            train_loss, train_loss2 = self.train_epoch(step_one=True)
            end_time = timer()
        ...
        self.model.brain_encoder.require_grad = False
        # self.model.text_encoder.require_grad = False

        for epoch in tqdm(range(1, self.args.num_first_step_epochs + 1), desc="first step epoch", ncols=0):
            start_time = timer()
            train_loss, train_loss2 = self.train_epoch(step_one=False, pretrain=True)
            end_time = timer()
        ...
        for epoch in tqdm(range(1, self.args.num_epochs + 1), desc="second step epoch", ncols=0):
            ...
            if self.best_tva_result is None or \
                ...
                self.best_tva_pred = tva_preds
                self.best_tva_probas = tva_probas
                self.best_hidden_state = self.hidden_state
                self.save_model()
        ...
        wandb.log({f"Best/Best {k}": v for k, v in self.best_tva_result.items()})
        wandb.log({f"Best/Best {k}": v for k, v in self.best_brain_result.items()})
        print("训练用时：", f"{(t_end_time - t_start_time) / 60:.3f}分钟")
        self.plot_confusion_matrix(modalities="all")
        self.plot_precision_recall(modalities="all")
        if self.data_config.num_classes == 2:
            self.plot_roc(modalities="all")
    ...
    def binary_evaluate(self):
        ...
        with torch.no_grad():
            ...
            tva_result['Loss/Testing Reconstruction Loss'] = reconstruction_loss / len(loss_list)
            tva_result['Loss/Testing KL Loss'] = kl_loss / len(loss_list)
