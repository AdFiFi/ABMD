...
class BMCLTrainer(Trainer):
    ...
    def init_model_config(self):
        if self.args.modality == 'vision-brain':
            ...
            bmcl_config = BMCLConfig(brain_config=brain_config,
            ...
            model = ModifiedBMCL(config=bmcl_config,
            ...
            bmcl_config = BMCLConfig(brain_config=brain_config,
                                     ...
                                     use_temporal=self.args.use_temporal)
            # model = ModifiedBMCL(config=bmcl_config,
            model = ModifiedBMCLTwoSteps(config=bmcl_config,
                                         brain_encoder=brain_encoder,
    ...
    def train_epoch(self, step_one=False):
        ...
        for step, inputs in enumerate(train_dataloader):
            ...
            wandb.log({'Training loss': loss.item(),
                       'Training Sim Loss': outputs.loss_sim.item() if outputs.loss_sim is not None else 0,
                       'Training Diff Loss': outputs.loss_diff.item() if outputs.loss_diff is not None else 0,
    ...
    def train(self):
        ...
        for epoch in tqdm(range(1, self.args.num_epochs + 1), desc="second step epoch", ncols=0):
            ...
            msg = f" Train loss: {train_loss:.5f}, Sim: {train_loss2[0]:.5f}, Diff: {train_loss2[1]:.5f}, " \
                  f"Test loss: {self.tva_result['Testing Loss']:.5f}, Sim: {self.tva_result['Testing Sim Loss']:.5f}, " \
                  f"Diff: {self.tva_result['Testing Diff Loss']:.5f}," \
    ...
    def binary_evaluate(self):
        ...
        with torch.no_grad():
            for inputs in evaluate_dataloader:
                ...
                loss = outputs.loss
                losses += loss.item()
                loss_diff += outputs.loss_diff.item()
                loss_sim += outputs.loss_sim.item()
                loss_list.append(loss.item())
            ...
            tva_result['Testing Loss'] = losses / len(loss_list)
            tva_result['Testing Sim Loss'] = loss_sim / len(loss_list)
            tva_result['Testing Diff Loss'] = loss_diff / len(loss_list)
