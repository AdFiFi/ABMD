...
class BMDTrainer(Trainer):
    ...
    def prepare_inputs_kwargs(self, inputs):
        ...
        if self.args.text_encoder in ["MTAMTextEncoder"]:
            ...
            if self.args.brain_encoder in ["MTAMEEGEncoder"]:
                ...
                if self.data_config.dataset in ["MNRED2"]:
                    return {"text": tokens.to(self.device),
                            "attention_mask": attention_mask,
                            "brain_signals": inputs["time_series"].to(self.device),
                            "labels": inputs["labels"].to(self.device)
    ...
    def init_model_config(self):
        if self.args.modality == 'vision-brain':
            ...
            bmd_config = BMDConfig(brain_config=brain_config,
            ...
            model = BMDTwoSteps(config=bmd_config,
                                brain_encoder=brain_encoder,
                                vision_encoder=vision_encoder)
            return model, bmd_config
    ...
    def train_epoch(self, step_one=False):
        ...
        for step, inputs in enumerate(train_dataloader):
            ...
            loss_parallel += outputs.loss_parallel.item() if outputs.loss_parallel is not None else 0
            loss_orthogonal += outputs.loss_orthogonal.item() if outputs.loss_orthogonal is not None else 0
            wandb.log({'Loss Main/Training loss': loss.item(),
                       'Loss/Training Parallel Loss': outputs.loss_parallel.item() if outputs.loss_parallel is not None else 0,
                       'Loss/Training Orthogonal Loss': outputs.loss_orthogonal.item() if outputs.loss_orthogonal is not None else 0,
                       'Loss Main/Learning rate': self.optimizer.param_groups[0]['lr']})

        return losses / len(loss_list), (loss_parallel / len(loss_list), loss_orthogonal / len(loss_list))
    ...
    def train(self):
        ...
        for epoch in tqdm(range(1, self.args.num_epochs + 1), desc="epoch", ncols=0):
            ...
            msg = f" Train loss: {train_loss:.5f}, " \
                  f"Parallel: {train_loss2[0]:.5f}, " \
                  f"Orthogonal: {train_loss2[1]:.5f}, " \
                  f"Test loss: {self.tva_result['Loss Main/Testing Loss']:.5f}, " \
                  f"Parallel: {self.tva_result['Loss/Testing Parallel Loss']:.5f}, " \
                  f"Orthogonal: {self.tva_result['Loss/Testing Orthogonal Loss']:.5f}," \
    ...
    def two_step_train(self):
        ...
        # self.model.reset_brain_decomposer()
        # self.model.reinit_text_decomposer()
    ...
    def binary_evaluate(self):
        ...
        with torch.no_grad():
            for inputs in evaluate_dataloader:
                ...
                loss = outputs.loss
                losses += loss.item()
                loss_orthogonal += outputs.loss_orthogonal.item()
                loss_parallel += outputs.loss_parallel.item()
                loss_list.append(loss.item())
            ...
            tva_result['Loss/Testing Parallel Loss'] = loss_parallel / len(loss_list)
            tva_result['Loss/Testing Orthogonal Loss'] = loss_orthogonal / len(loss_list)
